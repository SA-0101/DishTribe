...
function NearestRestourents() {
  ...
  useEffect(() => {
    if (coords.latitude && coords.longitude && mapRef.current && !leafletMapRef.current) {
      ...
      script.onload = () => {
        // Initialize map
        leafletMapRef.current = L.map(mapRef.current).setView([coords.latitude, coords.longitude], 15);
        ...
        const marker = L.marker([coords.latitude, coords.longitude]).addTo(leafletMapRef.current);
        ...
        if (nearestres.length > 0) {
          nearestres.forEach((restaurant) => {
            if (restaurant.latitude && restaurant.longitude) {
              const restaurantMarker = L.marker([restaurant.latitude, restaurant.longitude])
                .addTo(leafletMapRef.current)
                .bindPopup(`
                  <div style="text-align: center; min-width: 200px;">
                    <h3 style="margin: 0 0 8px 0; font-size: 14px; font-weight: bold;">${restaurant.restaurantName}</h3>
                    <p style="margin: 0 0 8px 0; font-size: 12px; color: #666;">${restaurant.address}</p>
                    <div style="display: flex; justify-content: center; margin-bottom: 8px;">
                      <span style="font-size: 12px;">Rating: ${restaurant.rating}/5</span>
                    </div>
                  </div>
                `);
            }
  ...
  }, [coords.latitude, coords.longitude, nearestres]);
  ...
  const getLocationName = async (latitude, longitude) => {
    try {
      ...
      if (data && data.display_name) {
        ...
        // Hide location name after 5 seconds
        setTimeout(() => {
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 md:px-6 md:py-12">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 mb-12 overflow-hidden">
          <div className="bg-gray-50 px-6 py-4 border-b border-gray-200">
            ...
            <p className="text-sm text-gray-600 mt-1">Map showing your current position - Click on your location marker to see address</p>
          </div>
          <div className="p-6 relative">
            ...
            {/* Location name popup */}
            {showLocationName && (
              <div className="absolute top-8 left-8 bg-white border border-gray-200 rounded-lg shadow-lg p-4 max-w-md z-50">
                <div className="flex items-start justify-between">
                  <div>
                    <h3 className="text-sm font-semibold text-gray-900 mb-1">Current Location</h3>
                    <p className="text-sm text-gray-600">{locationName}</p>
                  </div>
                  <button
                    onClick={() => setShowLocationName(false)}
